...
TestWebosExtensionServerThread::TestWebosExtensionServerThread()
    : Thread(kMockThreadName),
      ...
      output_(base::BindRepeating(
          &TestWebosExtensionServerThread::OnTestOutputMetricsFlush,
...
bool TestWebosExtensionServerThread::Start() {
  ...
  if (output_.GetPhysicalSize().IsEmpty())
    output_.SetPhysicalAndLogicalBounds(gfx::Rect(0, 0, 800, 600));
...
TestOutput* TestWebosExtensionServerThread::CreateAndInitializeOutput() {
  auto output = std::make_unique<TestOutput>(base::BindRepeating(
      &TestWebosExtensionServerThread::OnTestOutputMetricsFlush,
      base::Unretained(this)));
...
void TestWebosExtensionServerThread::OnTestOutputMetricsFlush(
