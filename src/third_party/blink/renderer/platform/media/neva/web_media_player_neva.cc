...
WebMediaPlayerNeva::WebMediaPlayerNeva(
  ...
  std::optional<bool> is_audio_disabled = GetClient()->IsAudioDisabled();
...
void WebMediaPlayerNeva::LoadMedia() {
  ...
  pending_request_.pending_load_ = std::nullopt;
#endif
...
void WebMediaPlayerNeva::Play() {
  ...
  pending_request_.pending_play_ = std::nullopt;
...
void WebMediaPlayerNeva::Seek(double seconds) {
  ...
  pending_request_.pending_seek_time_ = std::nullopt;
...
void WebMediaPlayerNeva::SetRate(double rate) {
  ...
  pending_request_.pending_rate_ = std::nullopt;
...
void WebMediaPlayerNeva::SetPreload(Preload preload) {
  ...
  pending_request_.pending_preload_ = std::nullopt;
  switch (preload) {
...
std::optional<media::VideoFrame::ID> WebMediaPlayerNeva::CurrentFrameId()
    const {
  NEVA_DCHECK(main_task_runner_->BelongsToCurrentThread());
...
bool WebMediaPlayerNeva::HasReadableVideoFrame() const {
  return false;
}
...
void WebMediaPlayerNeva::OnVideoWindowDestroyed() {
  video_window_info_ = std::nullopt;
