...
class WebMediaPlayer {
 public:
  ...
  // 'timeline offset' and January 1, 1970 UTC. Use
  // base::Time::InMillisecondsFSinceUnixEpoch() for generate this value from
  // Chromium code. If the content does not have a 'timeline offset' then
  // std::numeric_limits<double>::quiet_NaN() should be returned.
 ...
 protected:
  struct PendingRequest {
    std::optional<bool> pending_load_;
    std::optional<Preload> pending_preload_;
    std::optional<bool> pending_play_;
    // Seek gets pending if another seek is in progress or has no permit. Only
    // last pending seek will have effect.
    std::optional<base::TimeDelta> pending_seek_time_;
