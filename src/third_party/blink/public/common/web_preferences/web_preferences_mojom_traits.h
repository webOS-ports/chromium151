...
struct BLINK_COMMON_EXPORT
  ...
  // M151: EnumTraits::FromMojom returns the value (or std::optional for a
  // fallible conversion) instead of using an out parameter. Every mojom value
  // maps to a blink one here, so the conversion is infallible.
  static blink::FirstFramePolicy FromMojom(blink::mojom::FirstFramePolicy input);
};
#endif
...
struct BLINK_COMMON_EXPORT StructTraits<blink::mojom::WebPreferencesDataView,
  ...
  static bool css_navigation_enabled(const blink::web_pref::WebPreferences& r) {
  ...
  static bool keep_alive_webapp(const blink::web_pref::WebPreferences& r) {
