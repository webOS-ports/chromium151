...
#include "media/base/neva/sequenced_null_video_sink.h"
...
SequencedNullVideoSink::SequencedNullVideoSink(
    bool clockless,
...
void SequencedNullVideoSink::Start(RenderCallback* callback) {
  DCHECK(task_runner_->RunsTasksInCurrentSequence());
  DCHECK(!started_);
  callback_ = callback;
  started_ = true;
  last_now_ = current_render_time_ = tick_clock_->NowTicks();
  cancelable_worker_.Reset(base::BindRepeating(
      &SequencedNullVideoSink::CallRender, base::Unretained(this)));
  task_runner_->PostTask(FROM_HERE, cancelable_worker_.callback());
}

void SequencedNullVideoSink::Stop() {
  DCHECK(task_runner_->RunsTasksInCurrentSequence());
...
void SequencedNullVideoSink::CallRender() {
  DCHECK(task_runner_->RunsTasksInCurrentSequence());
  DCHECK(started_);
...
void SequencedNullVideoSink::PaintSingleFrame(scoped_refptr<VideoFrame> frame,
