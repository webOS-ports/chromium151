...
class CAPTURE_EXPORT VideoCaptureDeviceFactoryWebOS
 ...
 private:
  ...
  std::optional<VideoCaptureControlSupport> GetSupportedControl(
      const std::string& device_id);
  std::optional<VideoCaptureFormats> GetSupportedFormats(
      const std::string& device_id,
      base::Value* info);
  ...
  using VideoCaptureFormatsMap =
      base::flat_map<std::string, VideoCaptureFormats>;
  VideoCaptureFormatsMap supported_formats_cache_;
  using VideoCaptureControlSupportMap =
      base::flat_map<std::string, VideoCaptureControlSupport>;
  VideoCaptureControlSupportMap controls_cache_;
};
