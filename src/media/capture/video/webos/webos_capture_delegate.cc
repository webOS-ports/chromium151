...
mojom::RangePtr MojoRangeFromValue(base::DictValue* property) {
  if (!property)
...
VideoPixelFormat WebOSCaptureDelegate::GetPixelFormat(gfx::Size frame_size,
  ...
  base::DictValue& info_dict = info.GetDict();
  if (base::DictValue* formats =
...
void WebOSCaptureDelegate::OnCameraListUpdated(const std::string& response) {
  ...
  std::optional<base::DictValue> root_value =
