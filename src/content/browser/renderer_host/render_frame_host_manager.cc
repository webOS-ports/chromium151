...
bool RenderFrameHostManager::IsRendererTransferNeededForNavigation(
  ...
  if (ShouldSwapBrowsingInstancesForDynamicIsolation(rfh, dest_url_info))
    return true;
  ...
  if (ShouldProactivelySwapBrowsingInstance(
          dest_url_info, is_reload, is_same_site,
  ...
  auto current_site_info = SiteInfo::Create(
  ...
  if (opener && render_frame_host_->IsNavigationSameSite(dest_url_info) &&
...
RenderFrameHostManager::GetSiteInstanceForNavigationRequest(
  ...
  if (frame_tree_node_->IsMainFrame()) {
    ...
    no_renderer_swap_allowed |= !CanSubframeSwapProcess(
        ...
        request->dest_site_instance(), request, is_same_site);
  }
  if (no_renderer_swap_allowed && !should_swap_for_error_isolation)
    return scoped_refptr<SiteInstanceImpl>(current_site_instance);
...
bool RenderFrameHostManager::CanSubframeSwapProcess(
  ...
  if (!IsRendererTransferNeededForNavigation(
          render_frame_host_.get(),
          UrlInfo(
              UrlInfoInit(resolved_url)
                  .WithOriginIsolationRequest(UrlInfo::OriginIsolationRequest::
                                                  kOriginAgentClusterByHeader)),
          request, is_same_site)) {
