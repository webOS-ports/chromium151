  ...
  bool navigation_denied = false;
  {
    ...
    navigation_denied =
        ...
        !GetContentClient()->browser()->IsFileSchemeNavigationAllowed(
            GetURL(), frame_tree_node_->frame_tree_node_id().value(),
            commit_params_->is_browser_initiated);
...
#if defined(USE_NEVA_APPRUNTIME)
  // Let the embedder decide whether an HTTP error response gets its own error
  // page instead of being rendered.
...
#if defined(USE_NEVA_BROWSER_SERVICE)
  CHECK_NE(NavigationThrottle::BLOCK_BY_SITEFILTER, result.action());
  CHECK_NE(NavigationThrottle::BLOCK_BY_MALWARE_SITES, result.action());
#endif
...
net::IPEndPoint NavigationRequest::GetSocketAddress() {
  // TODO(523173951): CHECK-exclusion: Convert to a CHECK once we are confident
  // it won't be triggered.
