...
class BrowsingDataRemover {
 public:
  enum RemoveBrowsingDataMask {
    REMOVE_APPCACHE_DEPRECATED = 1 << 0,
    REMOVE_CACHE = 1 << 1,
    REMOVE_COOKIES = 1 << 2,
    ...
    REMOVE_SESSION_COOKIES = 1 << 20,
    REMOVE_PERSISTENT_COOKIES = 1 << 21,

    // "Site data" includes cookies, appcache, file systems, indexedDBs, local
    // storage, webSQL, service workers, cache storage, plugin data,
    // and statistics about passwords.
    REMOVE_SITE_DATA = REMOVE_COOKIES | REMOVE_FILE_SYSTEMS | REMOVE_INDEXEDDB |
                       REMOVE_LOCAL_STORAGE | REMOVE_PLUGIN_DATA |
                       REMOVE_SERVICE_WORKERS | REMOVE_CACHE_STORAGE |
                       REMOVE_WEBSQL | REMOVE_CHANNEL_IDS |
    ...
    REMOVE_ALL = REMOVE_SITE_DATA | REMOVE_CACHE | REMOVE_DOWNLOADS |
                 ...
                 REMOVE_MEDIA_LICENSES | REMOVE_CODE_CACHE,
  ...
  static BrowsingDataRemover* GetForStoragePartition(
 ...
 private:
  BrowsingDataRemover(content::StoragePartition* storage_partition);
