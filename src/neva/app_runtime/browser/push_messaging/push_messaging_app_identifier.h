...
//
// Copied from chrome/browser/push_messaging/push_messaging_app_identifier.h

#ifndef NEVA_APP_RUNTIME_BROWSER_PUSH_MESSAGING_PUSH_MESSAGING_APP_IDENTIFIER_H_
#define NEVA_APP_RUNTIME_BROWSER_PUSH_MESSAGING_PUSH_MESSAGING_APP_IDENTIFIER_H_
...
class PushMessagingAppIdentifier {
 public:
  ...
  // |profile|.
  static void DeleteAllFromPrefs(PrefService* pref_service);

  // Returns the number of PushMessagingAppIdentifiers currently registered for
  // the given |profile|.
  static size_t GetCount(PrefService* pref_service);

  ~PushMessagingAppIdentifier();

  // Persist this app identifier to prefs.
  void PersistToPrefs(PrefService* pref_service) const;

  // Delete this app identifier from prefs.
  void DeleteFromPrefs(PrefService* pref_service) const;
