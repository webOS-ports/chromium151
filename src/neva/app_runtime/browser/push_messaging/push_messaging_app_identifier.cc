...
PushMessagingAppIdentifier PushMessagingAppIdentifier::FindByAppId(
  ...
  const base::DictValue& map =
      pref_service->GetDict(kPushMessagingAppIdentifierMap);
...
PushMessagingAppIdentifier PushMessagingAppIdentifier::FindByServiceWorker(
  ...
  for (auto it : map) {
    if (it.second.is_string() &&
        base::StartsWith(it.second.GetString(), base_pref_value,
                         base::CompareCase::SENSITIVE)) {
      return FindByAppId(pref_service, it.first);
    }
  }
...
std::vector<PushMessagingAppIdentifier> PushMessagingAppIdentifier::GetAll(
  ...
  const base::DictValue& map =
      pref_service->GetDict(kPushMessagingAppIdentifierMap);
  for (auto it : map) {
    result.push_back(FindByAppId(pref_service, it.first));
  }
...
void PushMessagingAppIdentifier::DeleteAllFromPrefs(PrefService* pref_service) {
  pref_service->SetDict(kPushMessagingAppIdentifierMap, base::DictValue());
}

// static
size_t PushMessagingAppIdentifier::GetCount(PrefService* pref_service) {
  return pref_service->GetDict(kPushMessagingAppIdentifierMap).size();
}
...
void PushMessagingAppIdentifier::PersistToPrefs(
  ...
  ScopedDictPrefUpdate update(pref_service, kPushMessagingAppIdentifierMap);
  ...
  map.Set(app_id_,
          base::Value(MakePrefValue(origin_, service_worker_registration_id_,
                                    expiration_time_)));
}
