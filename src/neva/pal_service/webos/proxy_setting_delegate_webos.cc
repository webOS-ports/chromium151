...
void ProxySettingDelegateWebos::InitLunaServiceClient() {
  ...
  luna_service_client_ = luna::GetSharedClient(params);
}

void ProxySettingDelegateWebos::SubscribeProxyInforChange() {
  base::DictValue proxy_list_root;
...
void ProxySettingDelegateWebos::GetProxyInfoFromSettingsServiceCallback(
  ...
  std::optional<base::Value> response_value = base::JSONReader::Read(response, base::JSON_PARSE_CHROMIUM_EXTENSIONS);
  base::DictValue* dict = response_value->GetIfDict();
  if (!dict)
    return;
