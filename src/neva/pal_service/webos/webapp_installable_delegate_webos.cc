...
void WebAppInstallableDelegateWebOS::OnGetAppInfoStatus(
  ...
  if (status == pal::luna::Client::ResponseStatus::SUCCESS) {
    std::optional<base::Value> vals = base::JSONReader::Read(json, base::JSON_PARSE_CHROMIUM_EXTENSIONS);
    if (vals && vals->is_dict()) {
      std::optional<bool> return_value =
          vals->GetDict().Find("returnValue")->GetBool();
...
bool WebAppInstallableDelegateWebOS::SaveArtifacts(const WebAppInfo* app_info,
  ...
  base::DictValue appinfo_content;
...
bool WebAppInstallableDelegateWebOS::AreWebosIconsDifferent(
    ...
    const base::DictValue& appinfo_json,
    const base::FilePath& app_dir) {
  for (const auto& icon_type_and_name : Icon::icon_types) {
    ...
    icon_filename = appinfo_json.FindString(icon_name);
...
void WebAppInstallableDelegateWebOS::OnGetAppInfoPath(
  ...
  const std::string* folderPath =
      vals->FindStringByDottedPath("appInfo.folderPath");
  ...
  const std::string* current_ver =
      vals->FindStringByDottedPath("appInfo.version");
  if (current_ver)
  ...
  std::string* current_id = current_appinfo_json->FindString("id");
  ...
  std::string* current_title = current_appinfo_json->FindString("title");
  if (current_title) {
  ...
  std::string* current_url = current_appinfo_json->FindString("main");
  if (!current_url) {
    return std::move(callback).Run(true, fresh_app_info->version());
  }
  ...
  // background_color is std::optional<SkColor> in WebAppInfo
  std::string* background_color = current_appinfo_json->FindString("bgColor");
