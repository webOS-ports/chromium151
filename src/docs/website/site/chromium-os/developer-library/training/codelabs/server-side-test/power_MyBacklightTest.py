...
class power_MyBacklightTest(test.test):
    ...
    def _get_brightness(self):
        """Get brightness in integer value.

        It's not a percentage value and may be over 100.
        """
    ...
    def _set_als_disable(self):
        """Turns off ALS in power manager.

        Saves the old has_ambient_light_sensor flag if it exists.
        """
    ...
    def _restore_als_disable(self):
        """Restore the has_ambient_light_sensor flag setting.

        It was overwritten in _set_als_disable.
        """
    ...
    def run_once(self, client_ip):
        ...
        for test_name in self._transitions:
            ...
            self._results[test_name] = {
                'old': self._old_brightness,
                'new': self._get_brightness()
            }

    def cleanup(self):
        """Restore DUT before the test starts, and check the test results."""
        ...
        for test_name in self._results:
            ...
            if old_brightness == new_brightness:
                ...
                logging.info('   Brightness changed: %d -> %d', old_brightness,
                             new_brightness)
                num_failed += 1
