...
void WindowManagerWayland::ButtonNotify(unsigned handle,
                                        ...
                                        float y) {
  dragging_ =
      ((type == ui::EventType::kMousePressed) && (flags == ui::EF_LEFT_MOUSE_BUTTON));
...
void WindowManagerWayland::OnVirtualKeyNotify(EventType type,
                                              uint32_t key,
                                              int device_id) {
  keyboard_->OnKeyChange(key, 0, type != EventType::kKeyReleased, false,
...
void WindowManagerWayland::NotifyMotion(float x,
  ...
  MouseEvent mouseev(EventType::kMouseMoved,
                         position,
                         position,
                         EventTimeForNow(),
                         0,
...
void WindowManagerWayland::NotifyDragging(float x, float y) {
  gfx::Point position(x, y);
  MouseEvent mouseev(EventType::kMouseDragged, position, position, EventTimeForNow(),
...
void WindowManagerWayland::NotifyAxis(float x,
                                      ...
                                      int yoffset) {
  gfx::Point position(x, y);
  MouseEvent mouseev(EventType::kMousewheel, position, position, EventTimeForNow(), 0,
                     0);
...
void WindowManagerWayland::NotifyPointerEnter(uint32_t device_id,
  ...
  gfx::Point position(x, y);
  MouseEvent mouseev(EventType::kMouseEntered, position, position, EventTimeForNow(), 0,
                     0);
...
void WindowManagerWayland::NotifyPointerLeave(uint32_t device_id,
  ...
  // Disable dispatching EventType::kMouseExited event.
  gfx::Point position(x, y);
  MouseEvent mouseev(EventType::kMouseExited, position, position, EventTimeForNow(), 0,
                     0);
...
void WindowManagerWayland::NotifyTouchEvent(
  ...
  if (type == EventType::kTouchPressed)
  ...
  if (type == EventType::kTouchReleased || type == EventType::kTouchCancelled) {
    if (type == EventType::kTouchCancelled)
