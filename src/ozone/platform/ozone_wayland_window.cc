...
#if defined(USE_NEVA_APPRUNTIME)
class ScopedAllowBlockingForBitmap : public base::ScopedAllowBlocking {};
#endif
...
void CreateBitmapFromPng(
    ...
    base::OnceCallback<void(neva_app_runtime::CustomCursorType,
        SkBitmap*, int, int, bool)> callback) {
#if defined(USE_NEVA_APPRUNTIME)
  // This has to be called on the UI thread
  ScopedAllowBlockingForBitmap scoped_allow_blocking;
  ...
  base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
      FROM_HERE, base::BindOnce(std::move(callback), type, bitmap, hotspot_x,
                                hotspot_y, allowed_cursor_overriding));
#else
...
gfx::Rect OzoneWaylandWindow::GetBoundsInPixels() const {
  return bounds_;
}

void OzoneWaylandWindow::SetBoundsInPixels(const gfx::Rect& bounds) {
...
void OzoneWaylandWindow::SetBoundsInDIP(const gfx::Rect& bounds) {
  SetBoundsInPixels(delegate_->ConvertRectToPixels(bounds));
}

gfx::Rect OzoneWaylandWindow::GetBoundsInDIP() const {
...
void OzoneWaylandWindow::SetFullscreen(bool fullscreen,
...
void OzoneWaylandWindow::SetFullscreenWithSize(bool fullscreen,
                                               int64_t target_display_id,
                                               const gfx::Size& size) {
  if (size.width() == 0 || size.height() == 0) {
    SetFullscreen(fullscreen, target_display_id);
    return;
  }
  VLOG(1) << __PRETTY_FUNCTION__;
  SetBoundsInPixels(gfx::Rect(size.width(), size.height()));
  state_ = WidgetState::FULLSCREEN;
  SendWidgetState();
}
...
void OzoneWaylandWindow::SetCursor(scoped_refptr<PlatformCursor> cursor) {
  ...
  scoped_refptr<BitmapCursor> bitmap = BitmapCursor::FromPlatformCursor(cursor);
  bitmap_ = bitmap;
...
uint32_t OzoneWaylandWindow::DispatchEvent(
  ...
  DispatchEventFromNativeUiEvent(
      ne, base::BindOnce(&PlatformWindowDelegate::DispatchEvent,
...
void OzoneWaylandWindow::ValidateBounds() {
  ...
  gfx::Rect parent_bounds =
      window_manager_->GetWindow(parent_)->GetBoundsInPixels();
...
void OzoneWaylandWindow::SetRestoredBoundsInDIP(const gfx::Rect& bounds) {
  // TODO: https://crbug.com/848131
  NOTIMPLEMENTED();
}

gfx::Rect OzoneWaylandWindow::GetRestoredBoundsInDIP() const {
  // TODO: https://crbug.com/848131
  NOTIMPLEMENTED();
  return gfx::Rect();
}
...
void OzoneWaylandWindow::ResetCustomCursor() {
  if (bitmap_) {
    ...
    sender_->Send(new WaylandDisplay_CursorSet(std::vector<SkBitmap>(),
                                               ui::lsm_cursor_hide_hotspot));
  } else {
    // NOT_USE : Restore cursor(wayland cursor or IM's cursor)
    sender_->Send(new WaylandDisplay_CursorSet(std::vector<SkBitmap>(),
                                               ui::lsm_cursor_restore_hotspot));
  }
#endif
}
...
void OzoneWaylandWindow::SetCustomCursor(neva_app_runtime::CustomCursorType type,
  ...
  if (type == neva_app_runtime::CustomCursorType::kPath) {
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
        FROM_HERE,
        base::BindOnce(
            &CreateBitmapFromPng, type, path, hotspot_x, hotspot_y,
            allowed_cursor_overriding,
            base::BindOnce(&OzoneWaylandWindow::SetCustomCursorFromBitmap,
...
void OzoneWaylandWindow::SetCustomCursorFromBitmap(
  ...
  // mojom::CursorType argument of CreateImageCursor() reflects shape of cursor
  // so its role differs from neva_app_runtime::CustomCursorType which means
  // no_cursor/default_arrow/custom_arrow. This means neva custom cursor always
  // corresponds to mojom::CursorType::kPointer
  float scale = 1.0f;
  scoped_refptr<PlatformCursor> cursor =
      CursorFactory::GetInstance()->CreateImageCursor(
          mojom::CursorType::kPointer, *cursor_image,
          gfx::Point(hotspot_x, hotspot_y), scale);

  bitmap_ = BitmapCursor::FromPlatformCursor(cursor);
  window_manager_->SetPlatformCursor(nullptr);
  if (sender_->IsConnected())
    SetCursor();
...
void OzoneWaylandWindow::SetInputArea(const std::vector<gfx::Rect>& region) {
...
void OzoneWaylandWindow::SetContentsBounds() {
#if defined(OS_WEBOS)
