...
DesktopDragDropClientWayland::DesktopDragDropClientWayland(
    ...
    : root_window_(base::raw_ref<aura::Window>::from_ptr(root_window)),
      platform_window_(
          base::raw_ref<ui::PlatformWindow>::from_ptr(platform_window)),
...
ui::mojom::DragOperation DesktopDragDropClientWayland::StartDragAndDrop(
...
void DesktopDragDropClientWayland::UpdateDragImage(
...
void DesktopDragDropClientWayland::OnDragDrop() {
  ...
  if (delegate_) {
    ...
    auto drop_cb = delegate_->GetDropCallback(*event);
...
void DesktopDragDropClientWayland::IndicateDragWillBeAccepted() {
  ...
  platform_window_->DragWillBeAccepted(
      serial_, data_collector_->first_received_mime_type());
...
void DesktopDragDropClientWayland::OnDragDataCollected(
  ...
  if (event) {
    ...
    if (delayed_drop_location_) {
      ...
      auto data = std::make_unique<ui::OSExchangeData>();
      auto drop_cb = delegate_->GetDropCallback(*delayedEvent);
      auto output_drag_op = ui::mojom::DragOperation::kNone;
      std::move(drop_cb).Run(std::move(data), output_drag_op,
                             /*drag_image_layer_owner=*/nullptr);
      DragDropSessionCompleted();
...
DesktopDragDropClientWayland::CreateDropTargetEvent(
  ...
  aura::Window::ConvertPointToTarget(&root_window_.get(), target_window_,
                                     &target_location);
