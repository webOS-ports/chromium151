...
void DesktopWindowTreeHostOzone::Close() {
  ...
  if (!close_widget_factory_.HasWeakPtrs()) {
    ...
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
        FROM_HERE, base::BindOnce(&DesktopWindowTreeHostOzone::CloseNow,
...
void DesktopWindowTreeHostOzone::Show(ui::WindowShowState show_state,
  ...
  if (show_state == ui::SHOW_STATE_MAXIMIZED && !restore_bounds.IsEmpty())
    platform_window_->SetRestoredBoundsInDIP(ToPixelRect(restore_bounds));
...
void DesktopWindowTreeHostOzone::SetSize(const gfx::Size& requested_size) {
  ...
  gfx::Rect new_bounds = platform_window_->GetBoundsInPixels();
  new_bounds.set_size(size_in_pixels);
  platform_window_->SetBoundsInPixels(new_bounds);
#if defined(OS_WEBOS)
...
bool DesktopWindowTreeHostOzone::IsStackedAbove(aura::Window* window) {
  return false;
}
  ...
  if (native_shape) {
    ...
    if (!transform.IsIdentity() && !native_region.isEmpty()) {
      ...
      if (native_region.getBoundaryPath(&path_in_dip)) {
        path_in_dip.transform(gfx::TransformToFlattenedSkMatrix(transform), &window_mask);
      }
    }
...
void DesktopWindowTreeHostOzone::Maximize() {
  ...
  state_ |= Maximized;
  state_ &= ~Minimized;
  previous_bounds_ = platform_window_->GetBoundsInPixels();
...
void DesktopWindowTreeHostOzone::Restore() {
  ...
  if (!previous_bounds_.IsEmpty()) {
    platform_window_->SetBoundsInPixels(previous_bounds_);
...
void DesktopWindowTreeHostOzone::SetFullscreen(bool fullscreen,
  ...
  if (!(state_ & FullScreen)) {
    ...
    platform_window_->SetFullscreenWithSize(fullscreen, target_display_id,
                                            contents_size_);
#else
...
void DesktopWindowTreeHostOzone::SetBoundsInDIP(const gfx::Rect& bounds) {
  platform_window_->SetBoundsInDIP(bounds);
}
...
gfx::Transform DesktopWindowTreeHostOzone::GetRootTransform() const {
  ...
  display = display::Screen::Get()->GetDisplayNearestWindow(win);
...
DesktopWindowTreeHostOzone::GetKeyboardLayoutMap() {
  if (ui::LinuxUi::instance())
    return ui::LinuxUi::instance()->GetKeyboardLayoutMap();
  return {};
}
...
void DesktopWindowTreeHostOzone::OnBoundsChanged(
  ...
  OnHostResizedInPixels(platform_window_->GetBoundsInPixels().size());
  ResetWindowRegion();
}
...
gfx::Rect DesktopWindowTreeHostOzone::ToDIPRect(
  ...
  std::optional<gfx::RectF> rect_in_dip =
      GetRootTransform().InverseMapRect(gfx::RectF(rect_in_pixels));

  if (!rect_in_dip)
    return gfx::ToEnclosingRect(gfx::RectF(rect_in_pixels));
...
gfx::Rect DesktopWindowTreeHostOzone::ToPixelRect(
  ...
  rect_in_pixels = GetRootTransform().MapRect(rect_in_pixels);
...
void DesktopWindowTreeHostOzone::ResetWindowRegion() {
  ...
  SkPath window_mask;
  const gfx::Rect& bounds_in_pixels = platform_window_->GetBoundsInPixels();
...
void DesktopWindowTreeHostOzone::SetFirstActivateTimeout(
