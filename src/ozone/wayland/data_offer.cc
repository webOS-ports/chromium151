...
WaylandDataOffer::WaylandDataOffer(wl_data_offer* data_offer)
    : data_offer_(base::raw_ref<wl_data_offer>::from_ptr(data_offer)) {
...
WaylandDataOffer::~WaylandDataOffer() {
  wl_data_offer_destroy(&data_offer_.get());
}

void WaylandDataOffer::Accept(uint32_t serial, const std::string& mime_type) {
  wl_data_offer_accept(&data_offer_.get(), serial, mime_type.data());
}
...
int WaylandDataOffer::Receive(const std::string& mime_type) {
  ...
  wl_data_offer_receive(&data_offer_.get(), mime_type.data(), pipefd[1]);
  close(pipefd[1]);
  return pipefd[0];
}
