...
void WaylandTextInput::OnKeysym(void* data,
                                ...
                                uint32_t modifiers) {
  uint32_t key_code =
      ui::neva::KeycodeConverter::KeyNumberFromKeySymCode(key, modifiers);
  ...
  ui::EventType type = ui::EventType::kKeyPressed;
  WaylandDisplay* dispatcher = WaylandDisplay::GetInstance();
  ...
  ui::neva::IMEModifierFlags flag = ui::neva::FLAG_ALT;
  while (flag) {
    dispatcher->TextInputModifier(
        state, ui::neva::KeycodeConverter::GetModifierKey(
                   (ui::neva::IMEModifierFlags)(flag & modifiers)));
    flag = (ui::neva::IMEModifierFlags)(flag >> 1);
  }
