...
EGLConfig GLSurfaceWayland::GetConfig() {
  if (!config_) {
    EGLint config_attribs[] = {EGL_BUFFER_SIZE,
                               ...
                               EGL_WINDOW_BIT,
                               EGL_NONE};
    config_ = ui::ChooseEGLConfig(GetEGLDisplay(), config_attribs);
  }
  return config_;
}
...
void GLSurfaceWayland::OnFeedbackPresented(
  ...
  self->last_vsync_time_ =
      base::TimeTicks() + base::Microseconds(vsync_microseconds);
  TRACE_EVENT_INSTANT_WITH_TIMESTAMP0("wayland", "WaylandFeedbackPresented",
                                      TRACE_EVENT_SCOPE_GLOBAL,
                                      self->last_vsync_time_);
  if (refresh_nsec > 0)
    self->last_interval_ = base::Nanoseconds(refresh_nsec);
...
void GLSurfaceWayland::OnVSync(base::TimeTicks vsync_time,
  ...
  if (vsync_enabled_ && vsync_callback_) {
    constexpr base::TimeDelta kMinimumVSyncDelta = base::Milliseconds(2);
